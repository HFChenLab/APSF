...
def InputOptions():
	...
	options.add_argument('-S', '--sequential', action = 'store_true', help = 'add sequence-specific CMAP')
...
def main():
	...
	if env and sequential:
		print('%s: -e/--env and -S/sequential option are conflicted' % Color('Error', 'red'))
		exit()
	## print information
	StartInfor()
	...
	res_select_env = []
	check = 0
	if not silent and not sequential:
		while check < 1:
	...
	# check if sequential cmap parameter is consistent with the residue
	#  sequence in top file
	#print(cmap_name)
	#print(res_cmap)
	if sequential:
		if len(cmap_name) != len(res_cmap):
			print('%s: Your number of sequential CMAP are not consistent with the number of residues in %s file' % (Color('Error', 'red'), Color(prmtop, 'blue')))
			exit()
		check = 0
		for i in range(len(res_cmap)):
			if res_cmap[i] != cmap_name[i]:
				check += 1
		if check > 0:
			print('%s: Your sequential CMAP parameter are not consistent with the residue sequence in %s file' % (Color('Error', 'red'), Color(prmtop, 'blue')))
	...
	for i in range(len(res_cmap)):
		if sequential:
			...
			#print(len(res_cmap_unique))
			#print(len(res_cmap_unique_num))
	...
	for i in range(len(res_cmap)):
		...
		for j in range(int(res_pointer[num])-1, int(res_pointer[num+1])-1):
			if atom_name[j] == 'N':
				...
				cmap_dih_pointer.append(j+1)
		#
		if num+2 > len(res_pointer)-1:
			upper = len(atom_name)-1
		else:
			upper = int(res_pointer[num+2])-1
		for j in range(int(res_pointer[num+1])-1, upper): ##2019-09-14
			if atom_name[j] == 'N':
				cmap_dih_pointer.append(j+1)
		for k in range(len(cmap_dih_pointer)):
			cmap_add += '%8d' % cmap_dih_pointer[k]
		if sequential:
			cmap_add += '%8d\n' % (i+1)
		else:
